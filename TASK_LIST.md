...
## 3.3 Halaman Profil (/profil)
 ...
 - [ ] Styling Konten: Gunakan Tailwind Typography (prose) untuk body teks agar rapi. Judul text-3xl font-bold text-slate-900 border-b pb-4 mb-8. Foto pimpinan gunakan rounded-xl shadow-sm.

## 3.4 Halaman Layanan (/layanan dan sub-halaman)
 - [ ] Route GET /layanan — Layout: Grid dengan filter sidebar. Card layanan gaya "Dashboard App".
 - [ ] Route GET /layanan/kependudukan — List persyaratan menggunakan divide-y divide-slate-100 agar rapi tanpa border penuh.
 - [ ] Route GET /layanan/kesehatan — Tabel table-auto w-full bg-white rounded-xl overflow-hidden. Header bg-slate-50 text-slate-600 uppercase text-xs. Baris hover:bg-slate-50.
 - [ ] Route GET /layanan/kebencanaan — Kartu status: Gunakan badge warna solid (tanpa gradien). Contoh: bg-red-100 text-red-700 (Bahaya), bg-yellow-100 text-yellow-700 (Waspada).
 - [ ] Route GET /layanan/pariwisata — Galeri grid-cols-2 md:grid-cols-3 gap-4. Gambar rounded-xl. Modal Alpine dengan latar bg-black/50 backdrop-blur-sm.

## 3.5 Halaman Berita
 - [ ] Route GET /berita — Layout: Sidebar filter kiri (desktop), list kanan. Card berita tampilan list (horizontal) untuk mobile, grid untuk desktop.
 - [ ] Route GET /berita/<slug> — Artikel detail. Styling: prose prose-slate max-w-3xl mx-auto. Breadcrumb text-sm text-slate-500 mb-6. Gambar utama rounded-xl.

## 3.6 Halaman Pengaduan
 - [ ] Route GET /pengaduan — Layout: Form centered max-w-xl mx-auto. Label text-sm font-medium text-slate-700. Input mt-1 block w-full px-4 py-3 bg-white border border-slate-200 rounded-lg shadow-sm focus:ring-primary focus:border-primary.
 - [ ] Route POST /pengaduan — validasi error: text-red-500 text-xs mt-1.
 - [ ] Route GET /pengaduan/sukses/<id> — Konfirmasi: Ikon ceklis besar hijau, ID dengan font font-mono bg-slate-100 px-2 py-1 rounded.
 - [ ] Route GET+POST /pengaduan/cek — Hasil: Tampilan card status yang jelas. Badge status: px-3 py-1 rounded-full text-sm font-medium.

## 3.7 Halaman Error
 - [ ] 404.html & 500.html — Layout: Flexbox centered full height. Ikon Lucide w-24 h-24 text-slate-300. Teks text-slate-600. Tombol btn-primary untuk kembali. Hindari ilustrasi yang terlalu ramai.
